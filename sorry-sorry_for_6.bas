...
KEY1:
    IF Action_mode  = 0 THEN 'dancemode
        info_index = 0	'KOREA
        'GOSUB RBNV_info_1m9s
        GOSUB sorry_sorry
    ELSEIF Action_mode  = 1 THEN 'fightmode
        IF pose = 0 THEN
            GOSUB bow_1
        ENDIF
    ELSEIF Action_mode  = 2 THEN 'gamemode
        IF pose = 0 THEN
            GOSUB scissors
        ENDIF

    ELSEIF  Action_mode  = 3 THEN ' footballmode

        IF pose = 0 THEN
            GOSUB keeper_left
        ENDIF
    ELSEIF  Action_mode  = 4 THEN ' steeplechase_racesmode
        IF thing_chach_pose = 0 AND pose = 0 THEN
            GOSUB stair_left_up_3cm
        ENDIF
    ELSEIF  Action_mode  = 5 THEN ' cameramode
        IF pose = 0 THEN
            GOSUB flyingpose
        ENDIF
    ELSEIF  Action_mode  = 6 THEN ' MFpromote盖
        IF pose = 0 THEN
...
KEY2:
    ...
    ELSEIF  Action_mode  = 5 THEN ' cameramode
        IF pose = 0 THEN
            GOSUB fighter_pose
        ENDIF
    ELSEIF  Action_mode  = 6 THEN ' MFpromote
        IF thing_chach_pose = 0 AND pose = 0 THEN
...
action2:
    ...
    '  颤抖吧少年
    FOR I = 1 TO 4
	    MOVE G6A, 100,  90, 102, 132, 102,  
		MOVE G6D,  97,  88, 105, 132, 102,  
		MOVE G6B, 100, 100,  70,  ,  ,  
		MOVE G6C, 100, 100,  70,  ,  ,  
		WAIT

        DELAY 80
        MOVE G6A,  99, 105,  81, 136, 102,  
		MOVE G6D,  98, 102,  82, 137, 102,  
		MOVE G6B, 100,  79, 109,  ,  ,  
		MOVE G6C, 100,  79, 109,  ,  ,  
		WAIT
...
action3_1:
    ...
    FOR I = 1 TO 1
        'middle
        MOVE G6A,  98, 115,  68, 134, 103,  
		MOVE G6D,  97, 116,  66, 136, 103,  
       ...
       MOVE G6A,  98, 115,  68, 134, 103,  
        ...
        WAIT
        'left
        MOVE G6A, 102, 109,  80, 129,  95,  
		MOVE G6D,  94, 109,  77, 130, 109,  
		MOVE G6B, 190,  51,  19,  ,  , 90 
		MOVE G6C, 190,  18,  30,  ,  ,  
        WAIT
        
        MOVE G6A, 105,  95,  92, 133,  89,  
		MOVE G6D,  89,  91,  88, 141, 117,  
		MOVE G6B, 155,  51,  12,  ,  ,80  
		MOVE G6C, 151,  19,  30,  ,  ,  
        WAIT
          MOVE G6A, 102, 109,  80, 128,  95,  
		MOVE G6D,  94, 109,  77, 129, 109,  
		MOVE G6B, 190,  51,  19,  ,  ,90  
		MOVE G6C, 190,  18,  30,  ,  ,  
        WAIT
        '被试验MOVE G6A, 100, 111,  78, 129,  96,  
'		MOVE G6D,  93, 105,  76, 135, 108,  
'		MOVE G6B, 190,  51,  14,  ,  ,  
'		MOVE G6C, 190,  19,  30,  ,  ,  
'        WAIT
        'middle
    ...
    FOR I = 1 TO 2
        ...
        MOVE G6A,  84, 115,  79, 120, 111,
        MOVE G6D, 106, 116,  73, 123,  93,
        MOVE G6B, 169,  33,  14,  ,  , 135
        MOVE G6C, 190,  55,  54,  ,  ,
        WAIT


MOVE G6A,  96, 124,  55, 147,  99,  
MOVE G6D,  97, 126,  49, 148, 105,  
MOVE G6B, 181,  36,  19,  ,  ,  100
MOVE G6C, 181,  36,  19,  ,  ,  
WAIT

       '  被试验  MOVE G6A,  99, 123,  50, 144,  99,
'        MOVE G6D,  95, 119,  49, 150, 105,
'        MOVE G6B, 181,  36,  19,  ,  , 100
'        MOVE G6C, 181,  36,  19,  ,  ,
'        WAIT
 		MOVE G6D,  86, 119,  79, 116, 103,
        MOVE G6A, 106, 127,  73, 116,  97,
        MOVE G6C, 169,  33,  14,  ,  , 65
        MOVE G6B, 190,  55,  54,  ,  ,
        WAIT

'        MOVE G6A,  99, 123,  50, 144,  99,
'     被试验   MOVE G6D,  95, 119,  49, 150, 105,
'        MOVE G6B, 181,  36,  19,  ,  , 100
'        MOVE G6C, 181,  36,  19,  ,  ,
'        WAIT
        MOVE G6D,  94, 119,  55, 147,  99,  
MOVE G6A,  97, 124,  49, 150, 105,100  
MOVE G6B, 181,  36,  19,  ,  ,  100
MOVE G6C, 181,  36,  19,  ,  ,  
WAIT
...
action3_2: '抬脚
    ...
    'for unnumbered yellow one
	    ...
	    MOVE G6A,  97, 114,  66, 146, 103,  
		MOVE G6D,  97, 113,  64, 146, 102,  
		MOVE G6B, 170,  50,  80,  ,  ,  
		MOVE G6C, 170,  50,  80,  ,  ,  
		WAIT
	...
	FOR I = 1 TO 2
	    ' right
	    SPEED 10
	    MOVE G6A,  98, 114,  66, 146, 103,  
		MOVE G6D,  99, 106,  64, 153, 102,  
		MOVE G6B, 170,  50,  80,  ,  ,  
		MOVE G6C, 170,  50,  80,  ,  ,  
		WAIT

		MOVE G6A,  89,  97, 111, 116, 106,  
		MOVE G6D, 110, 100,  98, 127, 103,  
		MOVE G6B, 143, 121, 103,  ,  ,  
		MOVE G6C, 138, 107, 122,  ,  ,  
		WAIT
	    
	    MOVE G6A,  88, 107,  95, 122, 107,  
		MOVE G6D, 110, 100,  98, 127, 103,  
		MOVE G6B, 143, 121, 103,  ,  ,  
		MOVE G6C, 142,  60,  85,  ,  ,  
		WAIT
		...
		MOVE G6A, 111, 100, 100, 125, 105,  
		MOVE G6D,  88,  89, 118, 120, 103,  
		MOVE G6B, 138, 107, 122,  ,  ,  
		MOVE G6C, 143, 121, 103,  ,  ,  
		WAIT
	    
	    MOVE G6A, 112, 100,  98, 127, 103,  
		MOVE G6D,  86,  95,  99, 131, 107,  
		MOVE G6B, 142,  60,  85,  ,  ,  
		MOVE G6C, 143, 121, 103,  ,  ,  
		WAIT
		
	
		MOVE G6A, 112,  98,  98, 127, 103,  
		MOVE G6D,  83, 133,  48, 145, 111,  
		MOVE G6B, 142,  15,  51,  ,  ,  
		MOVE G6C, 143, 121, 103,  ,  ,  
		WAIT
